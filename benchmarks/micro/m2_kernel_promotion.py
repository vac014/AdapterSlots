#!/usr/bin/env python3
"""
fused_kernel_crossover_benchmark.py -- E13.6 Fused Triton kernel crossover benchmark + E13.7 MWC validation.
...
Usage:
    ...
    python scripts/experiments/fused_kernel_crossover_benchmark.py \\
        --output-dir results/kernel_promotion/e13_6_crossover/ \\
        --hardware a6000_tp1 \\
        --ranks 16 32 \\
    ...
    python scripts/experiments/fused_kernel_crossover_benchmark.py \\
        ...
        --output-dir results/kernel_promotion/e13_7_mwc/ \\
        --hardware a6000_tp1
...
def _fused_latency(X: torch.Tensor, W: torch.Tensor, A: torch.Tensor,
                   B: torch.Tensor, alpha: float,
                   n_reps: int, warmup: int, use_gpu: bool) -> float:
    """Measure fused LoRA latency using adapter_slots.kernel.FusedLoRAKernel."""
    ...
    for rank in ranks:
        ...
        if rank == ranks[0]:
            hw_profile_path = (
                ...
                / "adapter_slots" / "kernel" / "hw_profiles"
                / f"{hardware}.json"
            )
            hw_profile = {
                "hardware": hardware,
...
def main():
    ...
    parser.add_argument("--output-dir", default="results/kernel_promotion/e13_6_crossover/")
