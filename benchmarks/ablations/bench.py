...
async def _send_requests_async(
    ...
    # aiohttp defaults to TCPConnector(limit=100), silently capping in-flight
    # requests at 100 and making --max-num-seqs above that unreachable.
    # Applies equally to every backend; raise it so the SERVER is the bottleneck.
    _conn = aiohttp.TCPConnector(limit=int(os.environ.get("BENCH_CONN_LIMIT", "1024")))
