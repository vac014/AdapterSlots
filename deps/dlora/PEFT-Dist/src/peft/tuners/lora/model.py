...
from .layer import Conv2d, Embedding, Linear, LoraLayer, ColumnParallelLinear, RowParallelLinear
...
class LoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def _create_new_module(lora_config, adapter_name, target, **kwargs):
        ...
        elif isinstance(target, vllm_tp.ColumnParallelLinear):
            in_features, out_features = target.input_size, target.output_size
            new_module = ColumnParallelLinear(adapter_name, in_features, out_features, bias=bias, gather_output=target.gather_output, perform_initialization=False, **kwargs)
        elif isinstance(target, vllm_tp.RowParallelLinear):
            in_features, out_features = target.input_size, target.output_size
            new_module = RowParallelLinear(adapter_name, in_features, out_features, bias=bias, input_is_parallel=target.input_is_parallel, perform_initialization=False, **kwargs)
