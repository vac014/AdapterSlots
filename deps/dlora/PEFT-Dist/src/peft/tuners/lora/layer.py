...
import vllm.model_executor.parallel_utils.tensor_parallel as vllm_tp
...
class ColumnParallelLinear(vllm_tp.ColumnParallelLinear, LoraLayer):
    # Lora implemented in a dense layer
    def __init__(
        self,
        adapter_name: str,
        ...
        **kwargs,
        ...
        # this gets the init from nn.Linear's super perspective, i.e.
        # nn.Module.__init__, which should always be called
        vllm_tp.ColumnParallelLinear.__init__(self, in_features, out_features, bias=bias, gather_output=gather_output,
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if r > 0:
            self.lora_A[adapter_name] = nn.Linear(self.in_features, r, bias=False)
            self.lora_B[adapter_name] = nn.Linear(r, self.output_size_per_partition, bias=False)
            self.scaling[adapter_name] = lora_alpha / r
        if init_lora_weights:
            self.reset_lora_parameters(adapter_name)
    ...
    def _linear(self, input: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        return vllm_tp.ColumnParallelLinear.forward(self, input)
...
class RowParallelLinear(vllm_tp.RowParallelLinear, LoraLayer):
    # Lora implemented in a dense layer
    def __init__(
        self,
        adapter_name: str,
        ...
        **kwargs,
        ...
        # this gets the init from nn.Linear's super perspective, i.e.
        # nn.Module.__init__, which should always be called
        vllm_tp.RowParallelLinear.__init__(self, in_features, out_features, bias=bias, input_is_parallel=input_is_parallel,
                                      init_method=init_method, stride=stride, 
                                      keep_master_weight_for_test=keep_master_weight_for_test, skip_bias_add=skip_bias_add,
                                      params_dtype=params_dtype, use_cpu_initialization=use_cpu_initialization,
                                      perform_initialization=perform_initialization, reduce_results=reduce_results)
        # Note that we don't use self._init_empty_weights() for Linear because it is a bit slower and the benefit of
        # added robustness is not big enough for Linear.
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        # Actual trainable parameters
        if r > 0:
            self.lora_A[adapter_name] = nn.Linear(self.input_size_per_partition, r, bias=False)
            self.lora_B[adapter_name] = nn.Linear(r, self.out_features, bias=False)
            self.scaling[adapter_name] = lora_alpha / r
        if init_lora_weights:
            self.reset_lora_parameters(adapter_name)
    ...
    def _linear(self, input: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        return vllm_tp.RowParallelLinear.forward(self, input)
