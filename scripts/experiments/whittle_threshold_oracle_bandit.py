"""
whittle_threshold_oracle_bandit.py -- E8 Bandit Experiment: Whittle vs. Threshold vs. Oracle (whittle_scheduler, §5.1)

Primary experiment for whittle_scheduler (AB4 in the ablation plan).

Compares three dispatch policies in a simulated multi-adapter Poisson environment:
    Threshold -- |Q_k| >= W OR age >= T_max^(k) (V21 baseline from erlang_scheduler)
...
Cross-hardware usage -- same script, same logic, hardware determined by tau-iter-ms:
  ...
  Single RTX A6000 (TP=1, K ∈ {4,8,16}, primary):
    python scripts/experiments/whittle_threshold_oracle_bandit.py \\
        ...
        --output results/whittle_scheduler/a6000_single/e8_bandit_results.csv
  ...
  Two RTX A6000 PCIe (TP=2, K=4, delta_t calibration first via §5.5a):
    python scripts/experiments/whittle_threshold_oracle_bandit.py \\
        ...
        --output results/whittle_scheduler/two_a6000_pcie/e8_bandit_results.csv
  ...
  Two H100 NVLink (TP=2, K ∈ {4,8,16,32}, delta_t calibration first via §5.6a):
    python scripts/experiments/whittle_threshold_oracle_bandit.py \\
        ...
        --policies threshold whittle oracle \\
        --output results/whittle_scheduler/two_h100_nvlink/e8_bandit_k4_32.csv
  ...
  delta_t calibration comparison (§5.5a -- wrong vs. correct delta_t):
    python scripts/experiments/whittle_threshold_oracle_bandit.py \\
        ...
        --output results/whittle_scheduler/two_a6000_pcie/delta_t_calibration.csv
  ...
  AB4 ablation -- Threshold+Erlang+PI vs. Whittle+Erlang+PI (§5.4):
    python scripts/experiments/whittle_threshold_oracle_bandit.py \\
        ...
        --output results/whittle_scheduler/a6000_single/ab4_ablation.csv
  ...
  Fairness test (§5.3 / §5.5d) -- Whittle + Fair cap vs. Whittle + NoFair:
    python scripts/experiments/whittle_threshold_oracle_bandit.py \\
        ...
        --output results/whittle_scheduler/a6000_single/e8_fairness.csv
...
from adapter_slots.dispatch.oracle import OracleScheduler
from adapter_slots.dispatch.baselines import FIFODispatcher, GreedyFillDispatcher
...
def main() -> None:
    ...
    parser.add_argument("--erlang-tmax", action="store_true",
                        help="Use per-adapter Erlang T_max (erlang_scheduler); else use fixed 5-iter T_max")
    parser.add_argument("--pi-adaptive", action="store_true",
                        help="Enable PI-adaptive T_max (pi_controller); requires --erlang-tmax")
    ...
    parser.add_argument("--output", type=str, default="results/whittle_scheduler/e8_bandit_results.csv",
