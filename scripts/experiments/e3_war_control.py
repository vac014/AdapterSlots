"""
e3_war_control.py -- E3 WAR Control via T_max (impl_9, §8.2)
...
Usage:
  ...
  Single RTX A6000 (TP=1, K in {2,4,8}):
    python scripts/experiments/e3_war_control.py \\
        ...
        --output-dir results/impl_9/e3/a6000/

  Two RTX A6000 PCIe (staircase validation, Proposition 9.1):
    CUDA_VISIBLE_DEVICES=0,1 python scripts/experiments/e3_war_control.py \\
        --model ./models/llama-7b \\
        --adapter-dir ./adapters \\
        --K-values 4 \\
        ...
        --dataset-path ./data/sharegpt/sharegpt.jsonl \\
        --output-dir results/impl_9/e3/two_a6000_pcie/
  ...
  Two H100 NVLink (smooth curve, final paper):
    CUDA_VISIBLE_DEVICES=0,1 python scripts/experiments/e3_war_control.py \\
        ...
        --dataset-path ./data/sharegpt/sharegpt.jsonl \\
        --output-dir results/impl_9/e3/two_h100_nvlink/
...
def simulate_war_at_tmax(K, lam_total, tmax_eff_ms, tau_iter_ms, duration_s=300, seed=42):
    """
    Simulate WAR achieved at a given T_max_eff with the AS++ alignment buffer.
