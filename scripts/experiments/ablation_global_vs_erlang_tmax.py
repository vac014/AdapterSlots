"""
ablation_global_vs_erlang_tmax.py -- AB2 Ablation: Global T_max vs. Per-Adapter Erlang T_max

Experiment 6.1 / 6.5c from erlang_scheduler.md.

Runs two serving conditions at the same WAR* = 0.8 target and compares mean TTFT:
    GlobalT  -- single global T_max tuned to achieve WAR* = 0.8 (alignment_buffer system)
    ErlangT  -- per-adapter T_max^(k)* = Erlang_inv(W, lambda_k, WAR*) (erlang_scheduler)

Expected (Corollary 5.4):
...
def main():
    ...
    print(f"      (Use this to generate figures/tmax_vs_rank_k{args.K}.pdf)")
