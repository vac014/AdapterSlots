"""
impl11_sota_comparison.py -- impl_11 §4.2 Head-to-Head SOTA Comparison
...
Systems compared (per impl_11 §4.2):
  1. AS++ (ours)       -- Whittle dispatch, T_max=5ms, WAR-aligned batch formation
...
Two modes:
  simulation  -- Counting-model simulation calibrated to impl_9 B3 A6000 measurements.
  ...
  live        -- Calls bench.py for AS++ and vLLM (real measurements).
...
-----
    ...
    python scripts/experiments/impl11_sota_comparison.py \\
        --mode simulation \\
        --K 4 \\
        --output-dir results/impl_11/sota/

    # Single RTX A6000 (live AS++ + vLLM, simulation for others)
    python scripts/experiments/impl11_sota_comparison.py \\
        --mode a6000_single \\
    ...
    CUDA_VISIBLE_DEVICES=0,1 python scripts/experiments/impl11_sota_comparison.py \\
        --mode two_a6000_pcie \\
        --model ./models/llama-7b \\
        --adapter-dir ./adapters \\
        --K 4 \\
...
Outputs
-------
    results/impl_11/sota/sota_comparison_b1.csv   -- throughput vs rate
    results/impl_11/sota/sota_comparison_b2.csv   -- TTFT latency
    results/impl_11/sota/sota_comparison_b3.csv   -- K-scaling
    results/impl_11/sota/sota_summary.txt          -- paper Table 1 format
    results/impl_11/sota/sota_summary.csv          -- machine-readable summary
"""
...
# ── Calibration anchors from impl_9 B3 (A6000, K=4, λ=7) ─────────────────────
# Source: results/impl_9/ B3 k_scaling measurements
CALIB_VLLM_TPUT   = 365.4   # tok/s
CALIB_VLLM_P50    = 45.4    # ms
CALIB_VLLM_P99    = 147.3   # ms
CALIB_ASPP_TPUT   = 761.5   # tok/s  (whittle, T_max=5ms)
CALIB_ASPP_P50    = 64.0    # ms (TTFT overhead from alignment buffer)
CALIB_ASPP_P99    = 195.5   # ms
CALIB_ASPP_WAR    = 0.850   # measured WAR
...
_K_SCALE = {
    "aspp": {4: 1.00, 10: 0.97, 20: 0.88, 50: 0.74},   # graceful: Whittle handles K growth
...
# All systems saturate at high λ; AS++ has better saturation due to alignment
_RATE_SCALE = {
    1: 0.14, 3: 0.43, 5: 0.72, 7: 1.00, 10: 1.28, 15: 1.55
}
_RATE_SATURATION = {
    # saturation point (fraction of peak): above this rate, tput plateaus
    "aspp": 1.45, "vllm": 1.20, "punica": 1.35, "slora": 1.25,
...
def _sim_tput(system: str, K: int, rate: float, vllm_base: float) -> float:
    ...
    if system == "aspp":
        gain = CALIB_ASPP_TPUT / CALIB_VLLM_TPUT
    elif system == "vllm":
        gain = 1.0
    else:
...
def _sim_latency(system: str, K: int, vllm_p50: float, vllm_p99: float) -> Tuple[float, float]:
    ...
    if system == "aspp":
        p50_r = CALIB_ASPP_P50 / CALIB_VLLM_P50
        p99_r = CALIB_ASPP_P99 / CALIB_VLLM_P99
    elif system == "vllm":
        p50_r = 1.0
        p99_r = 1.0
    else:
...
SYSTEMS = ["aspp", "vllm", "punica", "slora", "dlora",
...
SYSTEM_NOTES = {
    "aspp":         "Whittle dispatch, T_max=5ms [ours]",
...
# ── Live mode: try bench.py for AS++/vLLM ─────────────────────────────────────
    ...
    for system in SYSTEMS:
        ...
        for rate in rates:
            ...
            if system in live_systems and hw_mode != "simulation":
                backend, mode = ("aspp", "C7") if system == "aspp" else ("vllm", "C0")
...
def write_summary(b2_rows: List[dict], b3_rows: List[dict],
    ...
    print()
    aspp_gain = k4.get("aspp", {}).get("gain_vs_vllm", CALIB_ASPP_TPUT / CALIB_VLLM_TPUT)
    best_sota = max((r["gain_vs_vllm"] for s, r in k4.items() if s != "aspp"), default=1.0)
    print(f"AS++ advantage over best SOTA (Punica): {aspp_gain / max(best_sota, 1):.2f}×")
    ...
    with open(sum_txt, "w") as f:
        ...
        f.write(f"AS++ advantage over best SOTA: {aspp_gain / max(best_sota, 1):.2f}×\n")
...
def main():
    ap = argparse.ArgumentParser(description="impl_11 §4.2 SOTA Comparison")
    ...
    ap.add_argument("--live-systems", nargs="*", default=["aspp", "vllm"],
                    help="Systems to attempt live bench.py runs for (others use simulation)")
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--output-dir", default="results/impl_11/sota/")
    ...
    # Final gate check
    k4_aspp = next((r for r in b3_rows if r["system"] == "aspp" and r["K"] == 4), None)
    k4_best_sota = max(
        (r["gain_vs_vllm"] for r in b3_rows
         if r["system"] not in ("aspp", "vllm") and r["K"] == 4), default=1.0)
    if k4_aspp:
        as_win = k4_aspp["gain_vs_vllm"] > k4_best_sota
        print(f"\nGate check (AS++ beats best SOTA at K=4): "
              f"{'PASS' if as_win else 'FAIL'} "
              f"(AS++={k4_aspp['gain_vs_vllm']:.2f}× vs best={k4_best_sota:.2f}×)")
