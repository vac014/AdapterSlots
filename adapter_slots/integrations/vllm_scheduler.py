...
_DEBUG_WGKP = os.environ.get("AS_DEBUG_WGKP", "0") == "1"
import pathlib
import time
import logging
...
class AlignmentAwareScheduler(Scheduler):
    ...
    def _schedule_running(self, budget, curr_loras, enable_chunking=False):
        ...
        """
        # Defer only when nothing is decodable. Under KV pressure preempted
        # sequences re-enter prefill on almost every tick, so deferring whenever
        # *any* sequence is in prefill starves decode indefinitely.
        if (_VLLM_AVAILABLE and self.running
                and os.environ.get("AS_NO_PREFILL_GUARD") != "1"
                and all(sg.is_prefill() for sg in self.running)):
            if _DEBUG_WGKP:
                import sys as _sys
            ...
            try:
                ...
                if self._schedule_call_count % 200 == 0:
                    ...
                    print(f"[AS_PROFILE_TICK] dumped {self._schedule_call_count} ticks to {path}",
                          file=_sys.stderr, flush=True)
        if os.environ.get("AS_DEBUG_SCHED", "0") == "1":
            return self._debug_wrap_schedule()
        if self._mode == "pi_adaptive":
    ...
    def _debug_wrap_schedule(self):
        """AS_DEBUG_SCHED=1: log why a tick produced an empty schedule."""
        import sys as _s, time as _t
        fn = {"pi_adaptive": self._schedule_pi_adaptive_inner,
              "wgkp": self._schedule_wgkp_inner,
              "erlang": self._schedule_erlang_inner,
              "whittle": self._schedule_whittle_inner,
              "sort": self._schedule_sort_inner}.get(
                  self._mode, self._schedule_threshold_inner)
        out = fn()
        n = len(getattr(out, "scheduled_seq_groups", []) or [])
        c = getattr(self, "_dbg_n", 0) + 1
        self._dbg_n = c
        if c <= 400 or c % 200 == 0:
            try:
                qs = {a: len(q) for a, q in self._buffer.queues.items() if len(q)}
                now = _t.perf_counter()
                ages = [round((now - t0) * 1000, 1)
                        for q in self._buffer.queues.values() for (_sid, t0) in list(q)[:1]]
                nrun = len(getattr(self, "running", []) or [])
                nwait = len(getattr(self, "waiting", []) or [])
            except Exception as e:
                qs, ages, nrun, nwait = f"ERR {e}", [], -1, -1
            print(f"[AS_DBG] tick={c} sched={n} running={nrun} waiting={nwait} "
                  f"queues={qs} oldest_ages_ms={ages[:6]}", file=_s.stderr, flush=True)
        return out
    ...
    def _record_overhead(self, t_start: float) -> None:
        ...
        if os.environ.get("AS_LOG_OVERHEAD") and len(self._tick_times_ms) % 200 == 0:
            import sys as _s, statistics as _st
            w = self._tick_times_ms[-200:]
            print(f"[AS_OVH] ticks={len(self._tick_times_ms)} "
                  f"mean={_st.mean(w):.2f}ms p50={_st.median(w):.2f}ms "
                  f"max={max(w):.2f}ms", file=_s.stderr, flush=True)
