...
class _AlignmentAwareMultiprocGPUExecutor(MultiprocessingGPUExecutor):
    """Same model_runner_cls injection as _AlignmentAwareGPUExecutor, for TP>1.

    Without this, any tensor-parallel run silently loses the entire AS kernel
    layer: LLMEngine._get_executor_cls() returns MultiprocessingGPUExecutor once
    tensor_parallel_size > 1, the `is GPUExecutor` test below fails, and the
    stock executor is used -- so AlignmentAwareModelRunner is never constructed
    and neither the grouped-GEMM nor the fused wrapper is installed. The
    scheduler still reorders batches, which makes the omission easy to miss:
    the server runs correctly and merely performs like stock vLLM.

    MultiprocessingGPUExecutor routes both the driver worker and every spawned
    worker through _get_create_worker_kwargs() -> _get_worker_kwargs(), so
    overriding this one method covers all ranks. AlignmentAwareModelRunner is a
    module-level class, so it pickles by reference into the spawned processes.
    """
...
class AlignmentAwareMQLLMEngine(MQLLMEngine):
    ...
    def from_engine_args(cls, engine_args, usage_context: UsageContext,
        ...
        elif executor_class is MultiprocessingGPUExecutor:
            executor_class = _AlignmentAwareMultiprocGPUExecutor
