...
class AlignmentAwareModelRunner(ModelRunner):
    ...
    def load_model(self) -> None:
        ...
        super().load_model()
        if self._wgkp_enabled:
            # Level-2 as a grouped GEMM over the adapter-sorted batch
            # (segmented_punica_wrapper.py); falls back to the packed-nslice
            # kernel below when unavailable.
            if os.environ.get("AS_SEGGEMM", "0") in ("1", "2"):
                from adapter_slots.kernel.segmented_punica_wrapper import (
                    install_segmented_punica_wrapper,
                )
                seg = install_segmented_punica_wrapper(self.lora_manager)
                print(f"[AS] install_segmented_punica_wrapper: installed={seg} "
                      f"min_tokens={os.environ.get('AS_SEGGEMM_MIN_TOKENS','192')}",
                      file=sys.stderr, flush=True)
                if seg:
                    return
